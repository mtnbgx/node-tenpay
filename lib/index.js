...
class Payment {
  constructor({appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox, certificate} = {}, debug = false) {
  ...
  async _parse(xml, type, signType) {
    let json = typeof xml === 'string' ? await util.parseXML(xml) : xml; 
  ...
  async _request(params, type, cert = false) {
    ...
    if (cert) {
      if (this.certificate) {
        pkg.key = this.certificate.key;
        pkg.cert = this.certificate.cert;
      } else {
        pkg.pfx = this.pfx;
      }
